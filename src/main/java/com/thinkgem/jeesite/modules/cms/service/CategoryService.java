...
public class CategoryService extends TreeService<CategoryDao, Category> {
	...
	public List<Category> findByUser(boolean isCurrentSite, String module) {
		...
		List<Category> list = null;//(List<Category>)UserUtils.getCache(CACHE_CATEGORY_LIST);
		if (list == null) {
		...
		*/
		...
		// 2019-04-12.lsp修改： 此处逻辑应该是，栏目的栏目模型=module时，列出其祖上所有的栏目
		if (isCurrentSite) {
			List<Category> categoryList = Lists.newArrayList();
			for (Category e : list) {
				if (Category.isRoot(e.getId()) || e.getSite() != null && e.getSite().getId() != null && e.getSite().getId().equals(Site.getCurrentSiteId())) {
					if (StringUtils.isNotEmpty(module)) {
						...
						if (module.equals(e.getModule())) {
							...
							if (parent != null && !parent.isRoot() && !categoryList.contains(parent)) {
								// 如果其parent存在且需要添加到树中，必须保证parent.parent不为null并且parent.parent.id不为空，让zTree初始化树时正常
								// logger.debug("parent.getName() ----------> {}，parent.getParent() ----------> {}", parent.getName(), parent.getParent());
								if (parent.getParent() == null || StringUtils.isBlank(parent.getParent().getId())) {
									parent = categoryDao.get(parent);
								}
	...
	public List<Category> findByParentId(String parentId, String siteId) {
		...
		site.setId(siteId);
		entity.setSite(site);
		entity.setInList(Global.SHOW);
		return dao.findByParentIdAndSiteId(entity);
	}

	public List<Category> findCategoryByArticle(String articleId) {
		return dao.findCategoryByArticle(articleId);
	}
	...
	public void save(Category category) {
		...
		CmsUtils.removeCache("mainNavListAndChild_" + category.getSite().getId());
	}
	...
	public List<Category> findMenuList(Category entityp) {

		return categoryDao.findMenuList(entityp);
	}
}
