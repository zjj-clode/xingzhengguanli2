...
public class LinkService extends CrudService<LinkDao, Link> {
	...
	@Transactional(readOnly = false)
	public Page<Link> findAllPage(Page<Link> page, Link link, boolean isDataScopeFilter) {
		// 更新过期的权重，间隔为“6”个小时
		Date updateExpiredWeightDate =  (Date)CacheUtils.get("updateExpiredWeightDateByLink");
		if (updateExpiredWeightDate == null || (updateExpiredWeightDate != null 
				&& updateExpiredWeightDate.getTime() < new Date().getTime())){
		...
		page.setCount(dao.findAllPageCount(link));
		link.setPage(page);
		page.setList(dao.findAllPage(link));
