...
public class ValidateCodeServlet extends HttpServlet {
	...
	public static boolean validate(HttpServletRequest request, String validateCode) {
		...
		if (code == null) {
			// return true; // 此时，每个session的第一次请求不需要验证码的
			return false;
		}
	...
	/**
	 * 默认验证码参数名称为 validateCode
	 */
	public static boolean validate(HttpServletRequest request) {
		String validateCode = request.getParameter(VALIDATE_CODE);
		return validate(request, validateCode);
	}

	/**
	 * 验证码验证成功后需要清除掉
	 */
	public static void clearValidateCode(HttpServletRequest request) {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (StringUtils.isNotBlank(validateCode)) {
			...
			response.getWriter().print(String.valueOf(validate(request, validateCode)));
		} else {
