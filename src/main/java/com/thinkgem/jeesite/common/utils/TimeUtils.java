...
public class TimeUtils {
	...
	private void parseTimeException(String time) {
		throw new IllegalArgumentException(time + ", time format error, HH" + timeSeparator + "mm" + timeSeparator
				+ "ss");
	}
	 ...
	 *            结束日期
	 * @param show
	 *            显示方法：1显示天，2显示天、小时，3显示天、小时、分钟，大于等于4时显示天、小时、分钟、秒。
	 * @return 1天2小时3分钟4秒
	 */
	public static String showTimePastStr(Date startDate, Date endDate, int show) {
		return showTimePastStr(endDate.getTime() - startDate.getTime(), show);
	}
	 ...
	 * @return 1天2小时3分钟4秒
	 */
	public static String showTimePastStr(long timePast, int show) {
		StringBuilder sb = new StringBuilder();
		if (show > 0) {
			long day = 0;
			long MILLISECONDS_DAY = 86400000;
			long day_left = timePast % MILLISECONDS_DAY;
			if (day_left == 0) {
				day = timePast / MILLISECONDS_DAY;
			} else {
				day = (timePast - day_left) / MILLISECONDS_DAY;
			}
			if (day > 0) {
				sb.append(day).append("天");
			} else {
			...
			if (show > 1) {
				...
				long hour_left = day_left % MILLISECONDS_HOUR;
				if (hour_left == 0) {
					hour = day_left / MILLISECONDS_HOUR;
				} else {
					hour = (day_left - hour_left) / MILLISECONDS_HOUR;
				}
				if (hour > 0) {
				...
				if (show > 2) {
					long min = 0;
					long MILLISECONDS_MIN = 60000;
					long min_left = hour_left % MILLISECONDS_MIN;
					if (min_left == 0) {
						min = hour_left / MILLISECONDS_MIN;
					} else {
						min = (hour_left - min_left) / MILLISECONDS_MIN;
					}
					if (min > 0) {
					...
					if (show > 3) {
						long sec = 0;
						long MILLISECONDS_SEC = 1000;
						long sec_left = min_left % MILLISECONDS_SEC;
						if (sec_left == 0) {
							sec = min_left / MILLISECONDS_SEC;
						} else {
							sec = (min_left - sec_left) / MILLISECONDS_SEC;
						}
						if (sec > 0) {
