...
<body>
	<div id="importBox" class="hide">
        <form id="importForm" action="${ctx}/xingzhengguanli/workArrangement/import" method="post" enctype="multipart/form-data" class="form-horizontal form-information" onsubmit="loading('正在导入，请稍等...');">
		    ...
		    <div class="row" style="padding-top:10px">
		      <div class="col-md-12">
		        <div class="form-group">
			      ...
			      <div class="col-sm-2">
			        <p class="help-block pull-left text-red"><a href="${ctx}/xingzhengguanli/workArrangement/import/template">下载模板</a></p>
			      </div>
		        </div>
		        <div class="form-group">
	...
	<section class="content-header">
      <h1>工作安排管理<small>工作安排信息</small></h1>
      <ol class="breadcrumb">
        <li><a href="${ctx}/home"><i class="fa fa-dashboard"></i> 首页</a></li>
        <li><a href="${ctx}/xingzhengguanli/workArrangement/">工作安排信息</a></li>
        <li class="active">信息列表</li>
      </ol>
    </section>
    <sys:message content="${message}"/>
    
    <form:form id="searchForm" modelAttribute="workArrangement" action="${ctx}/xingzhengguanli/workArrangement/" method="post" class="form-inline form-filter" role="form">
			...
			<%-- <div class="form-group"><label>开始时间：</label>
				<div class="input-group date">
		          ...
		          <input name="startDate" type="text" style="width: 230px" maxlength="20" class="form-control pull-right"
					value="<fmt:formatDate value="${workArrangement.startDate}" pattern="yyyy-MM-dd"/>"
			...
			<div class="form-group"><label>结束时间：</label>
				<div class="input-group date">
		          ...
		          <input name="endDate" type="text" style="width: 230px" maxlength="20" class="form-control pull-right"
					value="<fmt:formatDate value="${workArrangement.endDate}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true});"/>
		        </div>	
			</div> --%>
    ...
    <section class="content">
      <div class="row tableTopBtn">
        <div class="col-xs-12">
          	<shiro:hasPermission name="xingzhengguanli:workArrangement:delete">
          ...
          <!-- 	<button type="button" class="btn btn-success pull-right drModal"><i class="fa fa-upload" aria-hidden="true"></i> 批量导入</button> -->
          	<shiro:hasPermission name="xingzhengguanli:workArrangement:edit">
          	<a href="${ctx}/xingzhengguanli/workArrangement/form"><button type="button" class="btn btn-info pull-right"><i class="fa fa-plus" aria-hidden="true"></i> 添加工作安排</button></a>
          	</shiro:hasPermission>
        </div>
      </div>
      <div class="row">
          <div class="col-xs-12 simpleTable">
	            <shiro:hasPermission name="xingzhengguanli:workArrangement:delete">
	            <form action="${ctx}/xingzhengguanli/workArrangement/deleteBatch?queryString=${workArrangement.queryString}" method="post" id="batchDeleteForm">
	            </shiro:hasPermission>
	            <div style="overflow:auto;">
		            <table class="ui celled table_semantic" id="workArrangementListTable">
					<thead>
						<tr>
							<shiro:hasPermission name="xingzhengguanli:workArrangement:delete"><th width="20" class="checkboxTh"><input type="checkbox"></th></shiro:hasPermission>
							<th class="sort-column a.keshi">科室</th>
							<th>每周开始时间</th>
							<th>每周结束时间</th>
							<th>工作内容标题</th>
							<shiro:hasAnyPermissions name="xingzhengguanli:workArrangement:edit,xingzhengguanli:workArrangement:delete"><th>操作</th></shiro:hasAnyPermissions>
						</tr>
					</thead>
					<tbody>
					<c:forEach items="${page.list}" var="item">
						<tr>
							<shiro:hasPermission name="xingzhengguanli:workArrangement:delete"><td><input type="checkbox" name="ids" value="${item.id}"></td></shiro:hasPermission>
									<td>
										${item.keshi}
									</td>
									<td>
							...
							<shiro:hasAnyPermissions name="xingzhengguanli:workArrangement:edit,xingzhengguanli:workArrangement:delete">
							<td class="tablebox-handle">
			    				<shiro:hasPermission name="xingzhengguanli:workArrangement:edit">
			    				<a href="${ctx}/xingzhengguanli/workArrangement/form?id=${item.id}&queryString=${workArrangement.queryString}" class="btn btn-default btn-sm" title="编辑"><i class="fa fa-pencil"></i></a>
								</shiro:hasPermission>
								<shiro:hasPermission name="xingzhengguanli:workArrangement:delete">
								<a href="${ctx}/xingzhengguanli/workArrangement/delete?id=${item.id}&queryString=${workArrangement.queryString}" onclick="return confirmx('确认要删除该工作安排吗？', this.href)" class="btn btn-default btn-sm" title="删除"><i class="fa fa-close"></i></a>
								</shiro:hasPermission>
							</td>
			...
			<shiro:hasPermission name="xingzhengguanli:workArrangement:delete"></form></shiro:hasPermission>
              <!-- 分页 -->
              <div class="row">
                ${page}
              </div>
...
$(document).ready(function() {
	...
	$("#btnExport").click(function(){
		top.$.jBox.confirm("确认要导出工作安排吗？","系统提示",function(v,h,f){
			if(v=="ok"){
			   $("#pageSize").val(-1);
			   $("#searchForm").attr("action","${ctx}/xingzhengguanli/workArrangement/export");
			   $("#searchForm").submit();
			}
		},{buttonsFocus:1});
	...
	// 导出数据弹框
    var dcModalCont = '<p class="confirmTxt"><i class="fa fa-question-circle text-yellow"></i>确认要导出工作安排吗？</p>';
...
function page(n,s){
	$("#pageNo").val(n);
	$("#searchForm").attr("action","${ctx}/xingzhengguanli/workArrangement/");
	$("#pageSize").val(s);
	$("#searchForm").submit();
   	return false;
}
