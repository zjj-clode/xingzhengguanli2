...
<body>
	...
	<section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="${ctx}/qa/qaQuestion/">在线咨询信息</a></li>
        <li class="active">信息列表</li>
      </ol>
    </section>
    <sys:message content="${message}"/>
    
    <form:form id="searchForm" modelAttribute="qaQuestion" action="${ctx}/qa/qaQuestion/" method="post" class="form-inline form-filter" role="form">
    ...
    <section class="content">
      <div class="row tableTopBtn">
        <div class="col-xs-12">
            ...
            <a href="${ctx}/qa/qaQuestion/list1"><button type="button" class="btn btn-info pull-right">切换到问题管理页</button></a>
            </shiro:hasPermission>
          	...
          	<%-- <shiro:hasPermission name="qa:qaQuestion:edit">
          	<a href="${ctx}/qa/qaQuestion/createQuestionForm"><button type="button" class="btn btn-info pull-right"><i class="fa fa-plus" aria-hidden="true"></i> 添加咨询问题</button></a>
          	</shiro:hasPermission> --%>
          	<shiro:hasPermission name="qa:qaQuestion:edit">
				<a href="${ctx}/qa/qaQuestion/reIndex">
      ...
      <div class="row">
          <div class="col-xs-12 simpleTable">
	            <shiro:hasPermission name="qa:qaQuestion:delete">
	            <form action="${ctx}/qa/qaQuestion/deleteBatch?queryString=${qaQuestion.queryString}" method="post" id="batchDeleteForm">
	            </shiro:hasPermission>
				...
				<thead>
					<tr>
						<shiro:hasPermission name="qa:qaQuestion:delete">
						<th width="20" class="checkboxTh"><input type="checkbox" id="check"></th>
						</shiro:hasPermission>
						...
						<th class="sort-column a.state">是否回复</th>
						<th class="sort-column a.isopen">是否公开</th>
						<th class="sort-column a.hassendemail">已发送邮件</th>
				...
				<c:forEach items="${page.list}" var="item">
					<tr>
						<shiro:hasPermission name="qa:qaQuestion:delete"><td><input type="checkbox" name="ids" value="${item.id}"></td></shiro:hasPermission>
								...
								<td>
									${fns:getDictLabel(item.hassendemail, 'yes_no', '')}
								</td>
								<%-- <td>
						...
						<td class="tablebox-handle">
		    				...
		    				<shiro:hasPermission name="qa:qaQuestion:edit">
		    					<a href="${ctx}/qa/qaQuestion/form?id=${item.id}&queryString=${qaQuestion.queryString}" class="btn btn-default btn-sm" title="编辑"><i class="fa fa-pencil"></i></a>
							</shiro:hasPermission>
							
		    				<shiro:hasPermission name="qa:qaQuestion:edit">
		    					<%-- <a href="${ctx}/qa/qaQuestion/replyForm?question.id=${item.id}&queryString=${qaQuestion.queryString}" class="btn btn-default btn-sm" title="${item.state eq '1' ? '修改' :'' }回复"><i class="fa fa-reply"></i></a>
		    					 --%><a href="javascript:void(0);" onclick="toFrame('${item.id}')" class="btn btn-default btn-sm" title="${item.state eq '1' ? '修改' :'' }回复"><i class="fa fa-reply"></i></a>
							</shiro:hasPermission>
							
							<shiro:hasPermission name="qa:qaQuestion:delete">
								<a href="${ctx}/qa/qaQuestion/delete?id=${item.id}&queryString=${qaQuestion.queryString}" onclick="return confirmx('确认要删除该在线咨询吗？', this.href)" class="btn btn-default btn-sm" title="删除"><i class="fa fa-close"></i></a>
							</shiro:hasPermission>
    ...
    //iCheck for checkbox and radio inputs
  	...
  	.click(
  			function() {
  				...
  				$.jBox.confirm('<p class="confirmTxt"><i class="fa fa-question-circle text-yellow"></i>确认要批量删除选中的数据吗？</p>',
  								"系统提示",
  								function(v,h,f) {
  									if (v == "ok") {
  										$("#deleteAll").val(-1);
...
function page(n,s){
	$("#pageNo").val(n);
	$("#searchForm").attr("action","${ctx}/qa/qaQuestion/");
	$("#pageSize").val(s);
	$("#searchForm").submit();
   	return false;
}
...
//录取信息编辑弹窗
function toFrame(id){
    top.$.jBox.open("iframe:${ctx}/qa/qaQuestion/replyForm?question.id="+id, "招生咨询",1000,600,{
