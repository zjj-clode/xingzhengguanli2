...
<script type="text/javascript">
	...
	function ${input}SelectAction(fileUrl, data, allFiles){
		...
		for(var i=0; i<files.length; i++){
			...
			var fi = files[i].getThumbnailUrl();
			fi = cutContexPath(fi);
			url += fi;
			//</c:if>
			//<c:if test="${type ne 'thumb'}">
			var fi = files[i].getUrl();
			fi = cutContexPath(fi);
			url += fi;
			//</c:if>
	...
	function cutContexPath(url){
		var contexPath = '${pageContext.request.contextPath}';
		if(contexPath != "" && contexPath != "/" && url.indexOf(contexPath)==0){
			url = url.substring(contexPath.length);
		}
		return url;
	}
	...
	function ${input}ThumbSelectAction(fileUrl, data, allFiles){
		var url="", files=ckfinderAPI.getSelectedFiles();
		for(var i=0; i<files.length; i++){
			var fi = cutContexPath(files[i].getThumbnailUrl());
			url += fi;
			//
			if (i<files.length-1) url+="|";
		}//<c:if test="${selectMultiple}">
	...
	function ${input}Del(obj){
		// 判断删除的是第几个，删除${input}中对应的那个url
		var urls = $("#${input}").val().split("|");
		var newUrls = "";
		for (var i=0; i<urls.length; i++){
			if($(obj).attr("data-item")==i){//排除
				continue;
			}
			if (urls[i]!=""){
				newUrls+=urls[i]+"|";
	...
	function ${input}Preview(){
		...
		if(urls.length<2){
			$("#${input}Preview").css("list-style","none");
		}
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){
				//<c:if test="${type eq 'thumb' || type eq 'images'}">
				li = "<li><img src=\"${pageContext.request.contextPath}"+urls[i]+"\" url=\"${pageContext.request.contextPath}"+urls[i]+"\" style=\"max-width:${empty maxWidth ? 200 : maxWidth}px;max-height:${empty maxHeight ? 200 : maxHeight}px;_height:${empty maxHeight ? 200 : maxHeight}px;border:0;padding:3px;\">";
				//</c:if>
				//<c:if test="${type ne 'thumb' && type ne 'images'}">
				li = "<li><a href=\"${pageContext.request.contextPath}"+urls[i]+"\" url=\"${pageContext.request.contextPath}"+urls[i]+"\" target=\"_blank\">"+decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/")+1))+"</a>";
				//</c:if>
				li += "&nbsp;&nbsp;<c:if test="${!readonly}"><a href=\"javascript:\" data-item=\""+i+"\" onclick=\"${input}Del(this);\">×</a></c:if></li>";
