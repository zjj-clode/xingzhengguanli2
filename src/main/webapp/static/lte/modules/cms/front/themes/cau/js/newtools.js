(function($){
    ...
    $.fn.tabSwitch = function(options){
        ...
        this.each(function(){
            ...
            $this.find(options.tab).find("a").eq(options.active - 1).addClass("on");
            var eventState;
            ...
            var fn = eval(function() {
                ...
                if(eventState){
                    delayTime = setTimeout(function(){
                        $this.find(options.tab).find("a").removeClass("on");
                        $tab.addClass("on");
                        $this.find(options.tabcont).hide();
            ...
            //eval组合语句
            var init = eval("$this.find(options.tab).find('a')." + options.mouseevent + "(fn)");
            init;
        });
    };
